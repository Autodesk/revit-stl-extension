                ...
                for (int ii = 0; ii < mesh.NumTriangles; ii++)
                {
                    ...
                    try
                    {
                        ...
                        for (int n = 0; n < 3; ++n)
                        {
                            ...
                            {
                               xyz[3 * n] = ExporterIFCUtils.ConvertUnits(m_ActiveDocument, point.X, m_Settings.Units);
                               xyz[3 * n + 1] = ExporterIFCUtils.ConvertUnits(m_ActiveDocument, point.Y, m_Settings.Units);
                               xyz[3 * n + 2] = ExporterIFCUtils.ConvertUnits(m_ActiveDocument, point.Z, m_Settings.Units);
                            }
                            else
                            {
                               xyz[3 * n] = point.X;
